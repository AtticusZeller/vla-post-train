...
当前仅完成 benchmark 代码版本固定与远端登记，没有创建 `experiments/univtac/`、launcher、
配置或运行手册，也没有成功安装仿真依赖或下载数据。出现第一个明确实验后，再按具体
policy、task、预算和 seed 建立可复现配置；大型数据、checkpoint、视频和完整日志写入
