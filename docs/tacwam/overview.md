...
推理服务已具备完整调用链并与训练契约同源，但**尚无真实权重、GPU 或真机证据**——自动化测试
覆盖的是 websocket 传输契约、action chunk 消费、20 维动作还原、prompt 记忆化与相机几何，
全部基于 fake model。

这一层的局限在 2026-09-04 被上游对照真实数据集与 forward 的检查证实：假 model 能验证接线，
验证不了数值行为。当时查出两处训练/推理不一致，二者都是全绿测试下的静默偏差——训练把 50 步
动作块补零到 52 token 而服务端只采样 50；训练从 480x640 原图直接缩放到 224x320，而机体端默认
先 letterbox 成 224x224、服务端再缩一次，像素均值绝对差约 40、95% 像素改变。现已通过
`model.action_horizon = (video_num_frames - 1) * video_frame_stride` 与配置中的
`camera_image_size` 校验对齐，服务端在收到尺寸不符的帧时会报错并指明机体端应传的参数。
