...
晚介入窗口的 10 个成功标签里，9 个的人工步只有 2–11 步。如果把这 9 个算作自主成功，有标签口径会升到 37/79 = 46.8%。
本表没有这样算。
...
#### 2.4 待核实：低质量的"成功"数据会让行为变差

以下都是用户在真机上的观察，日志还不能直接验证：

- **（用户观察）** 连续把几次 actor 自主成功的数据录进去重新训练后，actor 开始失败，行为变得奇怪；
  再加入几次人类示教后又能成功。用户的判断是 actor 自主成功的数据质量不如人类示教，return=1 的低质量数据
  会把行为带坏。这里只讨论打了成功标签的数据。
- **（用户观察）** 不小心录入"插多了"的数据并标为成功（示教时插多，或 actor 自己往下插），actor 就会学会往下怼，
  actor 启动早时尤其明显。行为受示教数据质量和固定位置的影响很大。
- **（用户观察）** 前期示教之后，自主尝试几乎每次都能成功。日志里 ep24–60 的自主尝试为 4/8，滚动成功率最高
  出现在 ep122 前后和 ep217 前后（10 次中 6 次）。
- 失败数据对行为的影响还没有观察。

![前 3 个入库窗口中 actor 成功次数与下一次自主成功率](figures/0924-ufm32u22-recent-actor-success.png)

日志里对第一条做了粗检验：前 3 个入库窗口中 actor 成功（自主，或人工 ≤11 步的晚介入）≥2 次时，
下一次自主成功 5/19（26%），其余情况 23/51（45%），Fisher p=0.18；改看前 5 个窗口为 10/32 对 18/38，p=0.22。
方向与用户观察一致，样本不足以确认。"插多了"的数据日志里无法识别，因为没有记录插入深度。

replay 采样对全 buffer 均匀（`demo_buffer_ratio=0`，`replay_recency_half_life=null`），不会偏重最近的行，
所以原先"非常依赖最近的数据"的说法不成立。如果上面的现象存在，原因应在数据质量本身，而不是采样偏向近期。

- [ ] **未验证**：最近几次数据质量不高时，训练结果是否会变差。

#### 2.5 刚过 warm_up 时 actor 会抖，loss 降下来后不抖
