---
name: add-method
description: Onboard a method, framework, benchmark, backbone, or reproduction repository into the Agent Research Workspace. Use when adding or replacing a methods/* Git submodule, connecting an official upstream to a user-controlled fork, or registering a repository for future experiments.
---
...
1. Read root `AGENTS.md`, `README.md`, `docs/AGENTS.md`, `docs/workspace/overview.md`, `.gitmodules`,
...
5. Keep method Agent guidance method-specific. Preserve a useful upstream
   ...
   pointer or an existing symlink. Never copy the root workspace rules into a
   submodule.
6. Register the path, expected branch, and official upstream in
   `scripts/lab.py`; update the README role as `method`, `framework`,
   `benchmark`, `backbone`, or `reproduction`. Create
   `docs/<slug>/{plan,log,overview}.md` under the `docs/AGENTS.md` contract.
...
9. Validate the docs structure, this Skill, CLI method status, registry/
   `.gitmodules` agreement, and recursive submodules.
